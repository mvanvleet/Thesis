...
fig.text(0.5,0.035, 'Hellmann PES Energy (kJ/mol)',ha='center',
        va='center',
        fontweight='semibold',
        fontsize=24)
...
first=True
labels = []
label_texts = itertools.cycle(['3b Energies','Aniso-Iso FF','Aniso-Aniso FF'])
...
for i, (component_prefix,component_suffix) in enumerate(zip(component_prefixes,component_suffixes)):
    ...
    total_2b_energy_file = 'foo_2b_energy.dat'
    total_3b_energy_file = 'hellmann_3b_energy.dat'
    ...
    total_2b_energy = pd.read_csv(
                    total_2b_energy_file,delim_whitespace=True,names=['qm','unit'],skiprows=1)
    total_3b_energy = pd.read_csv(
                    total_3b_energy_file,delim_whitespace=True,names=['qm','unit'],skiprows=1)
